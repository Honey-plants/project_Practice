...
// ✅ 백엔드 라우터(API) 아직 미완성이라 임시로 주석 처리
// 나중에 라우터 완성되면 아래 주석만 해제하고,
// 아래 "STUB" 함수들은 삭제하면 됨.
// import { uploadReceipt, createReview } from "../../common/utils/reviewApi";
// import { uploadReviewImages } from "../../common/utils/reviewApi";
...
export default function ReviewWritePage() {
  ...
  /**
   * ✅ createReview STUB
   * - 실제론 payload를 서버에 POST해서 review_id를 받아야 함
   * - 지금은 review_id를 임의로 만들어 반환
   */
  async function createReviewStub(payload) {
    console.log("[STUB] createReview called with payload:", payload);

    // 실제 API 응답처럼 review_id를 돌려줌
    return { review_id: Math.floor(Math.random() * 100000) + 1 };
  }
  ...
  const submitReview = async () => {
    ...
    try {
      // ✅ 현재는 STUB 사용 (API 미완성)
      const result = await createReviewStub(payload);
  ...
  return (
    <div className="reviewWritePage">
      ...
      {step === "done" && <div>Review Created (check console logs)</div>}
    </div>
  );
}
