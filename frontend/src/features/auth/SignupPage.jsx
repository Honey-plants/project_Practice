...
import { signupMember, checkNickname as checkNicknameApi } from "../../common/utils/memberApi";
...
function SignupPage() {
  ...
  const toggleOption = (itemId) => {
    ...
    setSelections((prev) => {
      ...
      return {
        ...prev,
        [activeCategoryId]: exists ? current.filter((id) => id !== itemId) : [...current, itemId],
      };
    });
  };
  ...
  const checkNickname = async () => {
    ...
    try {
      ...
      setNickStatus("error");
      setNickMsg(e?.message || "Network error");
    }
  };
  ...
  const submitSignup = async () => {
    ...
    const payload = {
      ...
      dislike_tags: hateTags, // ✅ hate_input -> dislike_tags
    };
    ...
    <div className="signupPage">
      ...
      <section>
        ...
        <label>
          ...
          {nickStatus !== "idle" && <div>{nickMsg}</div>}
        </label>
            ...
            <div>
              ...
              {activeCategoryId === "hate" ? (
                <div>
                  ...
                  />
                  <button type="button" onClick={addHateTag} disabled={hateTags.length >= 3}>
                    Add
                  </button>
                  ...
                  <div style={{ marginTop: 6, fontSize: 12 }}>{hateTags.length}/3</div>

                  {/* 서버로 같이 보낼 값(디버그/유지용) */}
                  <input type="hidden" name="hateTags" value={JSON.stringify(hateTags)} />
                </div>
              ) : (
